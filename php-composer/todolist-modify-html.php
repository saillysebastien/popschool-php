...
$sql = sprintf("SELECT * FROM todo WHERE id=%s", $_GET["id"]);
$stmt = $conn->query($sql);
$task = $stmt->fetch();
  ...
  <div class="container-fluid">
    ...
    <form  class="form-horizontal center" action="<?= basename(__FILE__) ?>" method="post">
      ...
      <div class="form-group col-md-12">
        <input type="text" name="title" value="<?= htmlentities($task['title']) ?>" placeholder="Titre *" class="center"/>
      </div>
      ...
      <div class="form-group col-md-12">
        <input type="datetime" name="deadline" placeholder="AAAA-MM-JJ HH:MM:SS" value="<?= htmlentities($task['deadline']) ?>" class="center"/>
      </div>
